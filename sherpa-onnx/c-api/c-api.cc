...
static const std::vector<std::pair<std::string, std::string>>
    &GetWfloatAbbreviations() {
  static const std::vector<std::pair<std::string, std::string>> kAbbreviations =
      {
...
static int32_t ParsePlaceholderEnd(const std::string &s, size_t start,
...
static WfloatPlaceholderResult ProtectDecimals(const std::string &s) {
...
static WfloatPlaceholderResult ProtectEllipsis(const std::string &s) {
  WfloatPlaceholderResult ans;
  ans.text.reserve(s.size());
...
static std::vector<std::string> SplitRawSentences(const std::string &s) {
  ...
  while (i < n) {
    // Keep behavior identical to the Python implementation:
    // a sentence needs at least one character before a terminator.
    if (i == start) {
      ++i;
      continue;
    }
...
static std::string NormalizeEllipsis(const std::string &s) {
  std::string out;
  out.reserve(s.size());
...
static std::string NormalizeInterrobangClusters(const std::string &s) {
  std::string out;
  out.reserve(s.size());
...
static std::string RemoveDotBeforePunct(const std::string &s) {
  std::string out;
  out.reserve(s.size());
...
static std::string UppercaseFirstAsciiAlpha(const std::string &s) {
  std::string out = s;
...
static bool IsPunctOnlyChunk(const std::string &s) {
  size_t i = 0;
...
static bool EndsWithTerminalPunct(const std::string &s) {
  if (s.empty()) {
    return false;
  }
...
static WfloatPreparedText SplitIntoWfloatSentences(const std::string &text) {
...
static float ClampUnitFloat(float v) {
  if (!std::isfinite(v)) {
    return 0.0f;
  }
...
static std::string ResolveEmotionEmoji(const char *emotion) {
  ...
  std::string key = sherpa_onnx::ToLowerAscii(SHERPA_ONNX_OR(emotion, ""));
...
void SherpaOnnxOfflineTtsWfloatFreePreparedText(const char *prepared_text) {
